...
def get_data():
    ...
    df['is_data_science_professional'] = df.is_data_science_professional.map({1:'Sim',0:'Não'})
...
st.subheader('Quantidade de respostas a serem analisadas: %i' % (df["is_data_science_professional"].count()))
'''
## Você se considera um profissional que atua na área de Data Science?
'''

print(df['is_data_science_professional'])
...
soma = is_datascience[is_datascience["Resposta"] == 'Sim'].Valor.sum()
total = df["is_data_science_professional"].count()
print()

st.write("Aqui vemos que o percentual de entrevistados que se consideram **Data Scientists** é de **%.00f%%**." % ( (soma / total) * 100 ))

st.write("A partir de agora vamos trabalhar com um filtro de se o entrevistado é profissional de DataScience ou não")

datascience = st.multiselect("É profissional de DataScience?", list(df["is_data_science_professional"].unique()), ['Sim', 'Não'])


if len(datascience) > 0:
    df_dados_selecionados = df[df['is_data_science_professional'].isin(datascience)]

    df_living_state = pd.DataFrame(df_dados_selecionados["living_state"].value_counts())
    ...
    df_degreee_level = pd.DataFrame(df_dados_selecionados["degreee_level"].value_counts())
    ...
    df_salary_range = pd.DataFrame(df_dados_selecionados["salary_range"].value_counts())
    ...
    df_job_situation = pd.DataFrame(df_dados_selecionados["job_situation"].value_counts())
    ...
    df_salary_range = pd.DataFrame(df_dados_selecionados[df["job_situation"] == "Estagiário"].salary_range.value_counts())
